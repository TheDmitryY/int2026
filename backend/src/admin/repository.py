...
class AdminRepository:
    ...
    async def ban_user(self, user_id: uuid.UUID) -> None:
        ...
        query = (
            update(User).where(User.id == user_id).values(role="banned").returning(User)
        )
        result = await self.session.execute(query)
    ...
    async def unban_user(self, user_id: uuid.UUID):
        user = await self.get_by_id(user_id=user_id)
        query = (
            update(User).where(User.id == user_id).values(role="user").returning(User)
        )
        result = await self.session.execute(query)
        await self.session.commit()
        return result.scalar_one_or_none()
    ...
    async def unban_user_by_email(self, email: str):
        ...
        query = (
            update(User).where(User.email == email).values(role="user").returning(User)
        )
        result = await self.session.execute(query)
        await self.session.commit()
        return result.scalar_one_or_none()
    ...
    async def get_banned_user(self, user_id: uuid.UUID) -> User:
        query = select(User).where(User.role == "banned", User.id == user_id)
        result = await self.session.execute(query)
        return result.scalar_one_or_none()

    async def get_banned_users(self, limit: int, skip: int) -> List[User]:
        query = select(User).where(User.role == "banned").limit(limit).offset(skip)
        result = await self.session.execute(query)
        return result.scalars().all()
